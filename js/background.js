...
function addColabBtn() {
  chrome.tabs.getSelected(null, function(tab) {
    ...
    if(urlsplit[urlsplit.length-1] === 'ipynb'){
      let colaburl = tab.url.replace('https://github.com/','');
      let myscript=`
        if(!document.getElementById("colab")){
          (() => {
            ...
            curr.href = 'https://colab.research.google.com/github/'+'${colaburl}';
            curr.target= '_blank';
        ...
        // addColabBtn();
        // document.onload = addOpenColabBtn;
        `;
      chrome.tabs.executeScript(tab.id, {code: myscript});
  
    } else {
      console.log('Not a github iPython page')
    }
  });
}